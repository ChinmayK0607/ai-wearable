...
class VoiceToVoicePipeline:
    def __init__(
        ...
        Args:
            ...
            whisper_model_size (str): Size of the Whisper model ('tiny', 'base', 'small', 'medium', 'large').
        ...
        self.whisper_model = WhisperModel(whisper_model_size, device=self.device, compute_type="float16" if self.device == "cuda" else "int8")
    ...
    def generate_speech(self, text, output_path):
        ...
        # Initialize an empty byte array to concatenate audio segments
        full_audio = b""
        ...
        # Optionally, save the full audio as a single file
        sf.write(f"{output_path}.wav", full_audio, 24000)
        print(f"Speech generated and saved to {output_path}.wav")
